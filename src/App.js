...
function App() {
    ...
    const [likeText, setLikeText] = React.useState('Утримуйте, щоб відреагувати');
    const [newText, setNewText] = React.useState('Новий');
        ...
        <div className="wrapper">
                ...
                <div className="rating__stars">
                    {[...Array(5)].map((_, index) => (
                        <svg key={index} width="12" height="12" viewBox="0 0 12 12">
                            <path d="M6 .6L2.6 11.1 11.4 4.7H.6L9.4 11.1Z"
                                  fill={index + 1 > stars ? '#e3e3e3' : '#fabb05'}
                                  stroke={index + 1 > stars ? '#e3e3e3' : '#fabb05'}></path>
                        </svg>
                    ))}
                </div>
                <div className="rating__text">{time}</div>
                <div className="rating__new">{newText}</div>
            </div>
            <div className="review">{reviewText}</div>

            <div className="translate">Посмотреть перевод (русский)</div>
            <div className="like">
            ...
            {isOpen && (<div className="edit">
                ...
                <div className="edit__container">
                    ...
                    <label className="edit__label">
                        Количество отзывов:
                        <input className="edit__input" type="text" value={reviewCount}
                               onChange={(e) => setReviewCount(e.target.value)}/>
                    </label>

                    <label className="edit__label">
                        Звезды:
                        <input className="edit__input" type="number" value={stars} min="1" max="5"
                               onChange={(e) => setStars(e.target.value)}/>
                    </label>

                    <label className="edit__label">
                        Время:
                        <input className="edit__input" type="text" value={time}
                               onChange={(e) => setTime(e.target.value)}/>
                    </label>

                    <label className="edit__label">
                        Отзыв:
                        <textarea className="edit__textarea" value={reviewText}
                                  onChange={(e) => setReviewText(e.target.value)}></textarea>
                    </label>

                    <label className="edit__label">
                        Текст лайка:
                        <input className="edit__input" type="text" value={likeText}
                               onChange={(e) => setLikeText(e.target.value)}/>
                    </label>

                    <label className="edit__label">
                        Текст Новый:
                        <input className="edit__input" type="text" value={newText}
